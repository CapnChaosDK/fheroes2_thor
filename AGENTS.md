...
- The latest hardware-validated source implementation is `c636df9eea9347e9a2c52d4485be5740f274441d`.
- Show Artifacts / Show Army, native artifact artwork, tap and drag transfers, same-bag rearrangement, individual empty-slot moves and occupied-slot swaps, distinct scroll spells, spellbook protection, assembled sets, stats/scouting, cancellation, nested restoration, lifecycle, configurable system-respecting haptics, and all troop/whole-artifact/input regressions passed on the Thor. The validated APK remains at `android/app/build/outputs/apk/debug/app-debug.apk` when build outputs have not been cleaned. SHA-256: `6A40ABEB678C7C8A5419B0933E35A5DA442F0A01F4EC84F8F62699CEDCCBF629`.

## Next recommended planning point

- Hero Meeting artifact drag-and-drop and same-bag rearrangement are complete and hardware-validated. No next slice is approved. Multi-slot army redistribution, broader player-installed visuals, configurable layouts, left/right-handed modes, controls-only battery-saving mode, and additional haptic customization or long-press actions remain deferred alternatives.
