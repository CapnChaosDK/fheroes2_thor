...
- This checkpoint retains the upstream-through-`d778cb44b` integration and Thor CI safeguards, and adds a semantic lower-screen Close control for the post-battle Necromancy popup with exact Adventure restoration. Android build, app and isotools lint, explicit Thor installation and launch, and focused hardware validation passed. The debug APK SHA-256 is `56B9166C20AAC7C4F76745D03FD9A55019350DFF13A71AFB1D72B7AF8A29DCF1`.
...
- The latest hardware-validated source implementation is `dfc1a93ef`.
- Build and lint passed, the candidate installed and launched explicitly on the Thor, and the Necromancy Close control, exact Adventure restoration, upper-screen close path, physical controls, upstream integration, precise splitting, surrender retention, water-Hero Editor behavior, haptics, and dual-screen regressions passed. The validated debug APK remains at `android/app/build/outputs/apk/debug/app-debug.apk` when build outputs have not been cleaned. SHA-256: `56B9166C20AAC7C4F76745D03FD9A55019350DFF13A71AFB1D72B7AF8A29DCF1`.
