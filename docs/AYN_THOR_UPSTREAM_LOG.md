...
- Player-installed artifact artwork was added to both Hero Meeting bags at `8b7e67022`. The bounded 28-slot, 64 by 64 pixel native visual payload is revision-locked to inventory state, uses aspect-preserving Android rendering, and retains the text-only fallback. Publication avoids rebuilding unchanged inventory art.
- Native/Java identifier parity and MSVC artifact-request regression tests passed. Android assemble, app lint, and isotools lint passed through the short `R:` mapping. The APK installed over wireless ADB to `192.168.68.76:43311`, explicitly launched, resumed `org.fheroes2.thor:GameActivityProcess`, attached its presentation to display 4, and showed no sampled fatal or JNI-link error.
- The user passed all six focused hardware checks: visual matching and bounds, transfers/swaps and native effects, cancellation and haptic rules, lifecycle restoration, and the established troop, whole-artifact, upper-input, and physical-control regressions.
