...
- Hardware-validated source commit: `c636df9eea9347e9a2c52d4485be5740f274441d` (2026-09-13).
...
- A later hardware report found that the post-battle Necromancy popup did not publish a dialog context, leaving no lower-screen way to close it. The approved correction scopes that popup to the existing centered Battle Result Close deck and restores the previous context when it exits. Android assemble and lint pass through the required short `R:` mapping. The candidate installed successfully over wireless ADB and launched explicitly as `org.fheroes2.thor/org.fheroes2.GameActivity`; the activity is resumed without a matching Android runtime failure in the recent log. Debug APK SHA-256: `56B9166C20AAC7C4F76745D03FD9A55019350DFF13A71AFB1D72B7AF8A29DCF1`. Focused hardware validation passed: the centered lower-screen Close control appeared for Necromancy, closed the popup once, restored the Adventure controls, and retained the upper-screen and physical-control close paths.
...
- Closing Options or one of its nested dialogs restores the correct parent exactly once. Closing the complete Options flow restores the normal Adventure Map deck and its current focus, path, information, and enabled states.
- The approved nested-menu refinement gives Graphics, Audio, Interface, Language, Hot Keys, and Resolution their own semantic lower contexts. Graphics exposes Resolution, Mode, Scaling, V-Sync, System Info, and Back; Audio exposes Music, Effects, Music Type, 3D Audio, and Back; Interface exposes Type, Show/Hide, Army Estimate, Cursor, Scroll Speed, and Back. Language, Hot Keys, and Resolution provide bounded previous/next selection with Choose, Edit, Apply, Cancel, or Back as appropriate. Each screen publishes live values and restores its immediate native parent.
- The physical System Options hotkey, upper touchscreen, mouse, controller, marker privacy, haptics, expanded overview, and every established Adventure Map interaction remain unchanged.
- The first candidate incorrectly placed System Options on the expanded full-screen Adventure Overview. Hardware feedback clarified that it belongs on the normal Adventure Map command deck where hero movement occurs. The second candidate corrected the location, and hardware feedback then exposed the overlong label and generic lower-dialog fallback. The approved refinement shortens the command to Options and adds a complete dedicated context while preserving action identifier 237 for the correctly scoped Adventure action. The Thor identifier contract passes with 59 contexts and 248 actions. Android assemble, app lint, and isotools lint pass through the required short `R:` mapping. The refined APK installed successfully over wireless ADB and launched explicitly as `org.fheroes2.thor/org.fheroes2.GameActivity`; the activity is resumed, the companion window is attached to display 4, and the brief log check found no Android runtime, native fatal, or JNI-link failure. Refined debug APK SHA-256: `6E3B2095649F3B6CCA7830C516D4BE6F73EAC5C2FF35DDAB05C6F021523967DB`. Focused manual validation is pending.
- Hardware validation passed for the compact Options placement, complete parent deck, live parent values, direct parent actions, exact Adventure Map restoration, rapid input, lifecycle behavior, and established regressions. It exposed one remaining gap: nested Graphics, Audio, and related settings screens still fell back to generic lower buttons. The nested-menu refinement was approved on 2026-09-06.
- For the nested-menu candidate, the Thor identifier contract passes with 65 contexts and 277 actions. The compile-time same-context collision audit remains complete but is split into bounded groups to stay within Clang's constexpr evaluation limit. Android assemble, app lint, and isotools lint pass through the required short `R:` mapping. The candidate installed successfully over wireless ADB and launched explicitly; the game activity is resumed, the companion presentation is attached to display 4, and the brief log check found no Android runtime, native fatal, or JNI-link failure. Candidate debug APK SHA-256: `E3DC75FF2B8A512E05BA36BBBCCE75B993E14A30540D2ECBFBD4D2DFA35441F6`.
- All focused nested-menu checks passed on the Thor. Graphics, Audio, Interface, Language, Hot Keys, and Resolution exposed their proper lower controls and live values; bounded list navigation and edit/apply/cancel behavior worked; each nested screen restored its immediate parent; Options restored the exact Adventure Map state; and rapid input, lifecycle, upper-touchscreen, mouse, controller, privacy, haptic, and established gameplay regressions remained correct.

#### Focused Adventure Map System Options validation

1. On the normal Adventure Map, verify Options fits cleanly without clipping or obscuring existing controls and remains absent from the expanded full-screen map.
2. Open Options and verify the lower deck shows Language, Graphics, Audio, Hero Speed, Enemy Speed, Hot Keys, Interface, Text Support, Battles, and Okay / Back; verify its live values match the upper dialog.
3. Change both speeds, Text Support, and Battles from the lower screen; verify each native setting and live value update exactly once.
4. Open and close representative nested Language, Graphics, Audio, Hot Keys, and Interface screens; verify their proper lower controls appear and the complete Options deck restores afterward.
5. Close Options and verify the prior Adventure Map focus, plotted path, information, enabled actions, and deck restore. Exercise rapid taps, panel toggling, and suspend/resume with no duplicate dialog, stale action, stuck press, or stale information.
6. Recheck hero movement, Action, Adventure, File, Map, Heroes, Towns, the expanded overview, upper touchscreen, System Options hotkey, mouse, controller, marker privacy, and haptics.
