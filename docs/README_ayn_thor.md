...
The latest hardware-validated source implementation is `c636df9eea9347e9a2c52d4485be5740f274441d`. It adds artifact drag-and-drop and same-bag rearrangement to the player-installed artwork in both Hero Meeting bags, with touch-slop activation, bounded visual feedback, revision validation, safe cancellation, and configured haptics. Tap transfers, spellbook and assembly rules, statistics and scouting, lifecycle restoration, troop tools, and all established input paths remain unchanged. Android build, app/isotools lint, native artifact-request regressions, and identifier parity passed; the user passed all focused Thor hardware checks. The release APK SHA-256 is `6A40ABEB678C7C8A5419B0933E35A5DA442F0A01F4EC84F8F62699CEDCCBF629`. The latest published release is [`thor-v0.12.0`](https://github.com/CapnChaosDK/fheroes2_thor/releases/tag/thor-v0.12.0), sourced from validated checkpoint `2fe3bb0f9`. See [the backlog](AYN_THOR_BACKLOG.md) for validation history and deferred work.

The current release is hardware-validated. Upstream repairs, precise splitting, artifact tap and drag transfers, same-bag rearrangement, artifact artwork, complete Options and nested settings, lifecycle recovery, configurable system-respecting haptics, dual-screen presentation, upper touchscreen, mouse, and physical controls passed together. No new feature slice is approved.
