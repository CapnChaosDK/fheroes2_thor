...
The latest hardware-validated source checkpoint is `457e4fe10`. It provides the safe unknown-menu fallback plus the haptic, owned/allied/enemy/neutral-labeled, clustered, zoomable, presentation-filtered, fog-safe Expanded Adventure Map with privacy-aware marker information on top of semantic controls, read-only information cards, navigable game menus, both campaign selectors, the complete Editor workflow, touch-minimap viewport control, and hero and castle quick-selection lists. Its APK SHA-256 is `49D1C4EFB5BE1F4C4674B4E9CF5D687E6292ABA126E807BB9A3431B6751EA3EA`. The latest published prerelease remains [`thor-v0.7.0`](https://github.com/CapnChaosDK/fheroes2_thor/releases/tag/thor-v0.7.0), sourced from `4e396a814`.

The neutral-label checkpoint is hardware-validated. Visible individual neutral heroes and towns now receive native-authorized names at 2x and 4x, neutral labels yield last during collision layout, and fog, recruitment, capture, filters, clustering, input transparency, haptics, restoration, and established controls all passed focused testing. Select and approve the next focused slice before further implementation.
