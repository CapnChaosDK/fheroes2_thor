...
| Hero Meeting | Two five-slot armies with player-installed creature visuals; tap/drag move, merge, or swap; long-press exact stack splitting; whole-army and whole-artifact transfers/swaps; Show Artifacts / Show Army; individual artifact moves/swaps; Close |
...
The latest hardware-validated source checkpoint is `d9079c1d1039fbf4fb76c23f1fad7b998ebdcb34`. It adds individual Hero Meeting artifact transfers and swaps to every validated v0.10.0 workflow. Show Artifacts displays both 14-slot bags with names and scroll spells; tap a source and a slot in the other bag to move or swap through native inventory rules. Spellbooks are protected, stale requests are rejected, and nested/lifecycle restoration and all established troop/input paths passed hardware validation. Android build, app/isotools lint, native request regression tests, and identifier checks passed. The development APK SHA-256 is `35A526DF4CE4D6327BFC6A689200AE2061DB919E4481FD92F9F9D1F621E66DCA`. The latest published release remains [`thor-v0.10.0`](https://github.com/CapnChaosDK/fheroes2_thor/releases/tag/thor-v0.10.0), built from `9a7c595f2`; this newer development checkpoint is not a new release. See [the backlog](AYN_THOR_BACKLOG.md) for validation history and deferred work.

The current development checkpoint is hardware-validated. Upstream repairs, precise splitting, artifact transfers, complete Options and nested settings, lifecycle recovery, context restoration, haptics, dual-screen presentation, upper touchscreen, mouse, and physical controls passed together. The recommended next focused slice is individual Hero Meeting artifact manipulation; select and approve its exact behavior and tests before implementation.
