...
The latest hardware-validated source checkpoint is `490282958`. It provides the haptic, owned/allied/enemy-labeled, clustered, zoomable, presentation-filtered, fog-safe Expanded Adventure Map with privacy-aware marker information on top of semantic controls, read-only information cards, navigable game menus, both campaign selectors, the complete Editor workflow, touch-minimap viewport control, and hero and castle quick-selection lists. Its APK SHA-256 is `007414B348B7DC59D59F167D308BAF1B998C48FBE27650DD68E93CEE5B857B94`. The latest published prerelease remains [`thor-v0.7.0`](https://github.com/CapnChaosDK/fheroes2_thor/releases/tag/thor-v0.7.0), sourced from `4e396a814`.

The enemy-label checkpoint is hardware-validated. Native-authorized enemy hero and settlement labels appear at 2x and 4x after focused-owned, remaining-owned, and allied labels; clusters, 1x markers, and neutral markers remain unlabeled, while fog and live relationship changes withdraw enemy names through the revisioned snapshot path. Select and approve the next focused slice before further implementation.
