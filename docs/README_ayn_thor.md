...
The latest hardware-validated source checkpoint is `60cf8fe2d`. It provides the haptic, labeled, clustered, zoomable, presentation-filtered, fog-safe Expanded Adventure Map with privacy-aware marker information on top of semantic controls, read-only information cards, navigable game menus, both campaign selectors, the complete Editor workflow, touch-minimap viewport control, and hero and castle quick-selection lists. Its APK SHA-256 is `C42BCD784DC45F7626ECC9EBD49D636D249DFBE77D401156794244205C9322FF`. The latest published prerelease remains [`thor-v0.7.0`](https://github.com/CapnChaosDK/fheroes2_thor/releases/tag/thor-v0.7.0), sourced from `4e396a814`.

The next recommended planning point is privacy-safe labels for individually resolved allied heroes and settlements at 2x and 4x. Native code should publish label text only while the object is visible and allied; Android should reuse the validated owned-label layout while placing owned labels before allied labels. Clusters, 1x markers, enemies, and neutral markers remain unlabeled, and label text stays outside hit testing. Broader non-owned labels, sprites, drag-and-drop army management, configurable haptics, and configurable layouts remain separate later milestones. Present the exact behavior and focused Thor checks for approval before implementation.
