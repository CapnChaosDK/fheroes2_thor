...
The latest hardware-validated source checkpoint is `8b7e6702244a14acb548226fdef2df083c336bac`. It adds player-installed artifact artwork to both Hero Meeting bags, with bounded native sprites, aspect-preserving lower-screen display, revision-locked lifecycle safety, and text-only fallback. Existing individual artifact movement, swapping, scroll-spell naming, spellbook protection, haptics, troop tools, and all established input paths remain unchanged. Android build and app/isotools lint passed; the user passed focused Thor validation. The debug APK SHA-256 is `8AD7C063E3854F688DDD3D0008454B66DCE65D4DCF95CCE9A6CB97CB3B97CBF4`. The latest published release is [`thor-v0.11.0`](https://github.com/CapnChaosDK/fheroes2_thor/releases/tag/thor-v0.11.0), built from `8b7e67022`. See [the backlog](AYN_THOR_BACKLOG.md) for validation history and deferred work.

The current release is hardware-validated. Upstream repairs, precise splitting, artifact transfers and artwork, complete Options and nested settings, lifecycle recovery, configurable system-respecting haptics, dual-screen presentation, upper touchscreen, mouse, and physical controls passed together. No next slice is approved.
